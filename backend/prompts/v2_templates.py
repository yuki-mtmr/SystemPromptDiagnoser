...
3. **高度に個人化されたシステムプロンプト生成**: 認知特性から導出された「原則」に基づく3種類のプロンプトを生成
  ...
  "user_profile": {{
    ...
    "cognitive_profile": {{
      ...
      "formatting_principles": [
        "認知特性から導出された原則1（例：全体構造→詳細の順で説明）",
        "原則2（例：箇条書き・表・階層構造を活用）",
        "原則3（例：核心的情報を優先し、詳細は段階的に）"
      ],
...
### 【禁止事項】- 絶対に避けること
- 具体的な数値制約（「10-20トークン」「80-120語/段落」「3-7項目」等）
- 固定的なテーブル形式のフォーマットルール表
- 全ユーザー共通の定型的な構造
- サンプルプロンプトの丸写し

### 【推奨事項】- 積極的に取り入れること
- ユーザーの認知特性から導出された「原則」や「方針」
- 柔軟な表現（「簡潔に」「段階的に」「核心を優先」「詳細に」等）
- その人固有の好みや特性を反映したカスタム指示
- 実際に使いやすい、自然な文章

### プロンプトに含めるべき原則の例（認知特性に応じて選択）
- **思考パターン関連**: 「全体構造（マクロ）→要素（ミクロ）の順で説明」「具体例から一般化へ帰納的に説明」
- **構造化関連**: 「箇条書き・表・階層構造を用い、情報を整理」「流れるような文章形式で自然に説明」
- **詳細度関連**: 「核心的情報に絞り、詳細は追加質問を待って展開」「網羅的な情報を段階的に提示」「長文では適宜小結論を挟む」
- **トーン関連**: 「静かで知的なトーン、感情語や過剰比喩は避け」「専門家として的確なアドバイス」「対等な立場で議論」
- **レスポンス量関連**: 「初回応答は核心に絞り簡潔に」「必要に応じて詳細を追加」（数値は使わない）
...
### Standard スタイル（標準版）
- ペルソナ宣言 + 原則ベースの構造化ガイドライン + 認知特性
- 柔軟な表現で方針を記述

### Strict スタイル（詳細版）
- 完全なペルソナ宣言
- 詳細な原則リスト（数値ではなく方針として）
- 認知特性の詳細説明
- 回避パターンの完全なリスト
- インタラクション方針（質問への応答姿勢など）

## 生成されるプロンプトの品質基準

良いプロンプト例（原則ベース）:
```
私は全体構造の整合性が取れたときに理解が成立する、構造思考型の学習者です。

## 回答の原則
- 回答は常に全体構造（マクロ）→要素（ミクロ）の順で説明してください
- 箇条書き・表・階層構造を用い、情報を整理して提示してください
- トーンは静かで知的に、感情語や過剰な比喩は避けてください
- 初回応答は核心的情報に絞り、詳細は追加質問があれば展開してください

## 認知特性
- テキストの視覚的読み込みを好む
- 構造型・自律先行性・探索思考の傾向
- 過度な絵文字・装飾・スラングは認知負荷になるため避けてください
```

悪いプロンプト例（避けるべき固定ルール型）:
```
## 情報構造化の原則
| 要素 | ルール |
|------|--------|
...
| 箇条書き | 3-7項目 |
```
↑ このような具体的数値を含むテーブル形式は生成しないでください。

汎用的で表面的なプロンプト例（避けるべき）:
```
...
- 固定の数値ルールではなく、認知特性に基づいた柔軟な「原則」を生成してください
"""
